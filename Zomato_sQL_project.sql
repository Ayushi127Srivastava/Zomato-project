#Creating table and inserting date into it 
...
Question1-  What is the total amount each customer spent on Zomato?
...
Question2-  how many days has each customer visited zomato?
...
GROUP BY sales.userid
 ...
 Question3- What is the first product purchased by each customer ?

SELECT *
FROM(SELECT *,RANK() over(partition by userid order by created_date ) rnk FROM sales) a
WHERE rnk=1

Question4- What is the most purchased items on the menu and how many times it was purchased by all customer ?
...
order by number_of_times_purchased desc
 ...
 Question5- Which item was most popular for each customer ?
...
Question6=  Which item was first purchased by the customer after they become the member?
...
Question7-  Which item was purchased just before the customer become gold member ?
 ...
 Question8- What is the total order and amount spent for each member before they become member ?
...
Question9- If buying each product generates point like - product1 p1 , 5Rs=1Zomato point and p2 , 10rs=5 zomato points and p3, 5rs=1zomato points
...
ORDER BY total_sum_points desc
 ...
 Question10-  In the first one year after a customer joins the gold program (including their join date ) irrespective of what the customer has purchased they 
 ...
 Question11-  Rank all the transaction fo the customer ?
  ...
  Question12- Rank all the transaction for each customer whenever they are a zomato gold member for every non gold member trasaction mark as null
